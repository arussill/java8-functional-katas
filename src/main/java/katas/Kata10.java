...
public class Kata10 {
    public static List<Map> execute() {
        ...
        return lists.stream().map(
                elemento -> ImmutableMap.of(
                        "nombre", elemento.get("name"),
                        "videos", videos.stream()
                                .filter(video -> video.get("listId").equals(elemento.get("id")))
                                .map(element -> ImmutableMap.of("peliId", element.get("id"),
                                        "tituloPeli", element.get("title"))).collect(Collectors.toList()))).collect(Collectors.toList());
    }
}
